...
parser.add_argument('--input_dataset', type=str, choices=['cora', 'citeseer','pubmed'], default='pubmed',
                    help='The dataset to use. Options are "cora", "citeseer", and "pubmed".')
parser.add_argument('--seed', type=int, default=42, help='Random seed.')
parser.add_argument('--epochs', type=int, default=1800,
                    help='Number of epochs to train.')
parser.add_argument('--lr', type=float, default=0.0001,#0.0005
                    help='Initial learning rate.')
parser.add_argument('--weight_decay', type=float, default=.0,#5e-4,
                    help='Weight decay (L2 loss on parameters).')
parser.add_argument('--hidden', type=int, default=50,#50(pubmed),#100,#50,  #104
...
parser.add_argument('--hidden_GKAN', type=int, default=10,#10(pubmed),#20,#10,     ## <=15
                    help='Number of hidden units in GKAN architectures.')
parser.add_argument('--k_GKAN', type=int, default=2,
                    help='degree of polynomial in KAN.')
parser.add_argument('--g_GKAN', type=int, default=1, #<=======2
...
plt.plot([x+1 for x in range(args.epochs)], resutls_archit_2['train_acc'], 'bo-',label='GKAN Archit. 2',markersize=4, linewidth=1)  
...
plt.plot([x+1 for x in range(args.epochs)], resutls_archit_2['test_acc'], 'bo-',label='GKAN Archit. 2',markersize=4, linewidth=1)  
plt.plot([x+1 for x in range(args.epochs)], resutls_archit_1['test_acc'], 'go-',label='GKAN Archit. 1',markersize=4, linewidth=1)  
...
plt.plot([x+1 for x in range(args.epochs)], resutls_archit_2['train_loss'], 'bo-',label='GKAN Archit. 2',markersize=4, linewidth=1)
