...
def curve2coef(x_eval, y_eval, grid, k, device="cpu"):
    ...
    mat = mat.double()  # Convert to double precision
    y_eval = y_eval.double()
    
    ##  --------------- Regularization ------------------------
    threshold = 1e3  # Adjust based on your needs
    regularization_term = 1e-5
    I = torch.eye(mat.size(1), mat.size(2), device=mat.device, dtype=mat.dtype)  
    expanded_I = I.unsqueeze(0).repeat(mat.size(0), 1, 1)  
    mat += regularization_term * expanded_I
