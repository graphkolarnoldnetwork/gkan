...
parser.add_argument('--input_dataset', type=str, choices=['cora', 'citeseer','pubmed'], default='cora',
...
model_kan_archit_2_k2 = KAN(width=[input_feat_KAN, args.hidden_GKAN-5, labels.max().item() + 1], grid=args.g_GKAN, k=args.k_GKAN, noise_scale=0.1, seed=0,base_fun=torch.nn.ReLU(),symbolic_enabled=False,bias_trainable=True,device=device, architecture='2',original_input_dim = features.shape[1],dimension_transformation_layer = None, grid_eps=args.grid_eps)
model_kan_archit_2_k3 = KAN(width=[input_feat_KAN, args.hidden_GKAN-10, labels.max().item() + 1], grid=args.g_GKAN, k=args.k_GKAN, noise_scale=0.1, seed=0,base_fun=torch.nn.ReLU(),symbolic_enabled=False,bias_trainable=True,device=device, architecture='2',original_input_dim = features.shape[1],dimension_transformation_layer = None, grid_eps=args.grid_eps)
...
plt.plot([x+1 for x in range(args.epochs)], resutls_archit_2['train_acc'], 'bo-',label='GKAN Archit. 2 w. h='+str(args.hidden_GKAN),markersize=4, linewidth=1)  
plt.plot([x+1 for x in range(args.epochs)], resutls_archit_2_k2['train_acc'], 'mo-',label='GKAN Archit. 2 w. h='+str(args.hidden_GKAN-5),markersize=4, linewidth=1)  
plt.plot([x+1 for x in range(args.epochs)], resutls_archit_2_k3['train_acc'], 'ko-',label='GKAN Archit. 2 w. h='+str(args.hidden_GKAN-10),markersize=4, linewidth=1)  
...
plt.plot([x+1 for x in range(args.epochs)], resutls_archit_2['test_acc'], 'bo-',label='GKAN Archit. 2 w. h='+str(args.hidden_GKAN),markersize=4, linewidth=1)  
plt.plot([x+1 for x in range(args.epochs)], resutls_archit_2_k2['test_acc'], 'mo-',label='GKAN Archit. 2 w. h='+str(args.hidden_GKAN-5),markersize=4, linewidth=1)  
plt.plot([x+1 for x in range(args.epochs)], resutls_archit_2_k3['test_acc'], 'ko-',label='GKAN Archit. 2 w. h='+str(args.hidden_GKAN-10),markersize=4, linewidth=1)  
